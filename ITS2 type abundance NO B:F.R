...
data_long <- data_long %>%
  filter(!(ITS2_Profile %in% c("F5.1", "B1aw")))
...
ggplot(summary_data, aes(x = interaction(Location, Morph), y = MeanRelAbundance, fill = ITS2_Profile)) +
...
# Plot stacked bar chart
ggplot(summary_data, aes(x = Morph, y = MeanRelAbundance, fill = ITS2_Profile)) +
  geom_bar(stat = "identity") +
...
ggplot(summary_pie,
  ...
  geom_col(width = 1, alpha = 0.85) +
  geom_text_repel(data =summary_pie %>% filter(Fraction > 0.001), 
