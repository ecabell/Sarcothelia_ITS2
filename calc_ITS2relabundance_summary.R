...
# Load your data
file_path <- "~/Documents/Zoox PCR/20250603T195144_cabeler1/post_med_seqs/596_20250604T195602_DBV_20250605T195712.seqs.relative.abund_only.csv"  # Replace with your actual file path
...
# Total abundance
total_abundance <- sum(data, na.rm = TRUE)

# A3 columns and abundance
a3_cols <- grep("A3", colnames(data), value = TRUE)
a3_abundance <- sum(data[, a3_cols], na.rm = TRUE)
a3_percent <- (a3_abundance / total_abundance) * 100

# A-clade columns and abundance
a_cols <- grep("\\bA\\b|_A|^A", colnames(data), value = TRUE)
a_abundance <- sum(data[, a_cols], na.rm = TRUE)
a_percent <- (a_abundance / total_abundance) * 100

# Non-A3 A-clade columns and abundance
non_a3_a_cols <- setdiff(a_cols, a3_cols)
non_a3_a_abundance <- sum(data[, non_a3_a_cols], na.rm = TRUE)
non_a3_a_percent <- (non_a3_a_abundance / total_abundance) * 100

# Top 5 non-A3 A-clade types
non_a3_a_sums <- colSums(data[, non_a3_a_cols], na.rm = TRUE)
